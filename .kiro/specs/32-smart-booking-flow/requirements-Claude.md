...
>
> Discovered while implementing: the original spec's capacity model doesn't match how capacity
> actually needs to work, and is silent on an admin override capability. Agreed with the user
> in conversation (not from a KIRO spec update) — see `packages/server/tests` and this feature's
> `design-Claude.md` for the technical implementation this drove.
>
> **1. Capacity must be computed via real time-interval overlap, not exact `start_time` matches.**
> Example from the user: a 2-person 90min booking at 6:00pm, a 1-person 30min booking at 6:30pm,
> and a 1-person 60min booking at 7:00pm on the same 3-capacity resource — the room is at capacity
> from 6:30–7:30pm even though all three bookings have different start times. `Remaining_Capacity`
> (Requirement 1.3, 1.7, 1.8) and the `/availability/days` day-status logic (Requirement 3.7) must
> use `start_time < otherEnd AND end_time > otherStart` overlap checks, summed by headcount — not
> "same resource + same start_time."
>
> **2. `participant_count` must be persisted on the booking, reversing the original "no migration
> needed" decision** (see original Notes section below). Without storing it, a later overlap check
> has no way to know an existing booking occupies more than 1 spot. New migration adds
> `apt_bookings.participant_count INTEGER NOT NULL DEFAULT 1`.
>
> **3. New Requirement 6: Admin Capacity Override** (not in the original spec at all):
> - **User Story:** As a business owner/manager, I want to create a booking that exceeds a
>   resource's normal capacity (e.g. accommodating a walk-in), so that I'm not hard-blocked by a
>   limit an end-user booking should never be allowed to cross.
> - Reuses the existing `overrideRules` flag already accepted by `createBooking` (currently used
>   for lead-time and staff-working-hours bypass) rather than inventing a new one — it's already
>   gated behind the `bookings:*` permission at the route level (business owner/manager tier only;
>   never exposed to a customer-facing flow), and there's no existing UI control for it yet, so a
>   new checkbox is needed regardless of which flag backs it.
>   1. WHEN `overrideRules` is true on a booking creation request, THE Server SHALL skip the
>      capacity rejection (Requirement 1.8) and allow the booking to be created even if
>      `participant_count` exceeds `Remaining_Capacity`.
>   2. THE Server SHALL still record the actual `participant_count` on an override booking, so
>      future overlap/capacity calculations for that time window correctly reflect the real
>      (over-capacity) headcount.
>   3. THE Booking_Create_Page SHALL expose an "Override capacity limit" checkbox, visible
>      whenever the participant count field is visible, unchecked by default.
>   4. No new highlighting mechanism is stored on the booking — an over-capacity slot is a
>      *derived* state (summed participant headcount > resource capacity for that overlap group),
>      not a stored flag. See `34-capacity-aware-calendar`'s `design-Claude.md` for how the
>      calendar surfaces this.
>
