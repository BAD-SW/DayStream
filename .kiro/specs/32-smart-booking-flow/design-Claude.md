# Design Document (Claude-amended)

> ## ⚠️ Amendments to the original design.md (2026-08-06)
>
> See `requirements-Claude.md` in this folder for the "why." This section is the concrete
> technical plan replacing/extending the original design's §2 and §3.
>
> ### New migration: `apt_bookings.participant_count`
>
> ```sql
> -- 093_booking_participant_count.sql
...
> ```
>
> The original design explicitly said no migration was needed, treating `participant_count` as a
> transient validation parameter. That doesn't survive contact with real overlap checking: a later
> booking's capacity check needs to know how many spots *earlier* overlapping bookings already
> occupy, which requires the value to persist.
>
> ### §3 (Booking Service Extension) — replaced
>
> The original design's capacity check used `WHERE resource_id = $1 AND start_time = $2` (exact
> match) and a plain `COUNT(*)`. Replaced with a real interval-overlap query summed by headcount:
>
> ```sql
> SELECT COALESCE(SUM(participant_count), 0)::int AS booked
> FROM apt_bookings
> WHERE resource_id = $1
>   AND business_id = $2
>   AND status IN ('pending', 'confirmed', 'in_progress')
>   AND start_time < $3   -- new booking's end_time
>   AND end_time > $4     -- new booking's start_time
> ```
>
> `remaining = capacity - booked`. If `participantCount > remaining`:
> - `overrideRules` **not** set → throw (mapped to 409, same as original design).
> - `overrideRules` set → allow. The booking is still inserted with its real `participant_count`,
>   so `booked` for that window is now genuinely `> capacity` for anyone computing it afterwards —
>   this *is* the over-capacity signal, no separate flag needed.
>
> ### §2 (`/availability/days`) and `getAvailabilityCombinations` — additive change to
> `availability.service.ts`
>
> The original design listed `availability.service.ts` as absolutely must-not-touch. That
> conflicts with its own Notes section, which says `capacity_remaining` must be populated by "the
> availability service" — there's no other place the resource-overlap data needed to compute it
> exists. Resolution (discussed with the user): treat "must not touch" as "must not change which
> slots are considered available" (the actual behavioral contract), not "must not add a field to
> the return value using data the function already computes internally."
>
> Concretely, inside `getAvailabilityCombinations()`:
> - The resource-bookings query gains `participant_count` in its SELECT list.
> - The existing per-slot concurrency sweep (which already correctly walks overlap checkpoints —
>   see the `checkPoints`/`maxConcurrent` logic in the current code) is reused, but changed from
>   counting *bookings* to summing *participant_count* at each checkpoint, matching the same
>   overlap-then-sum logic as the booking-service check above.
> - `capacity_remaining = resourceCapacity - maxConcurrentParticipants` is computed per slot and
>   attached to the pushed `AvailableSlotCombo` (at both existing push sites). The slot-inclusion
>   gate itself changes from a boolean "resource fully booked" check to "`capacity_remaining <= 0`"
>   — behaviorally equivalent for the default case (participant_count 1), and now correct for
>   larger requested counts too.
> - No change to staff/location/hours logic, no change to the function's public signature
>   (`AvailabilityQuery` is untouched) — the route layer (not the service) is responsible for
>   comparing a requested `participant_count` against the now-correct `capacity_remaining`.
>
> ### Cross-reference: feature 31
>
> `34-capacity-aware-calendar`'s `slot_booking_count` has the identical exact-`start_time`-match
> flaw and needs the same overlap+SUM(participant_count) fix, plus a new calendar fill-state for
> "over capacity" (booked > capacity, distinct from "full" = booked === capacity). See that
> feature's own `design-Claude.md`.
>
> Everything below this point is the original, unmodified KIRO design.md.
