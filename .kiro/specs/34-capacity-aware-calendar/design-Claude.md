...
>
> See `requirements-Claude.md` in this folder and `32-smart-booking-flow`'s amended docs for the
> full "why." Concrete changes to what's already implemented:
>
> ### `booking-calendar.service.ts` — `slot_booking_count` query replaced
>
> Was (exact `start_time` match):
> ```sql
> SELECT COUNT(*)::int FROM apt_bookings b2
> WHERE b2.resource_id = b.resource_id AND b2.start_time = b.start_time
>   AND b2.business_id = b.business_id AND b2.status IN ('pending','confirmed','in_progress')
> ```
> Becomes (overlap + summed headcount):
> ```sql
> SELECT COALESCE(SUM(b2.participant_count), 0)::int FROM apt_bookings b2
> WHERE b2.resource_id = b.resource_id AND b2.business_id = b.business_id
>   AND b2.status IN ('pending','confirmed','in_progress')
>   AND b2.start_time < b.end_time AND b2.end_time > b.start_time
> ```
> (`b.end_time`/`b.start_time` are the outer booking row's own times — this correlated subquery
> now finds every active booking whose interval overlaps the outer row's interval, not just ones
> starting at the same instant, and sums their `participant_count` instead of counting rows.)
> Depends on the `participant_count` column added by `32-smart-booking-flow`'s migration
> `093_booking_participant_count.sql`.
>
> ### New 4th fill state: `'over-capacity'`
>
> `FillState` becomes `'available' | 'almost-full' | 'full' | 'over-capacity'`. `getFillState`
> rule: `count > capacity` → `'over-capacity'` (checked before the existing `full` branch, which
> becomes `count === capacity`). Gets its own CSS token — `--color-capacity-override` — rather
> than reusing `--color-error` (which already means "full/blocked" and shouldn't also mean
> "intentionally allowed past the limit"; those are different messages to a staff member reading
> the calendar). Added to `colors.css` `:root`, alongside the existing semantic status colors.
>
> This is what "highlighted in the calendar" (from the original conversation with the user) means
> concretely: no new column, no new flag — an over-capacity slot is *derived* purely from
> `slot_booking_count > resource_capacity`, which naturally happens whenever a `overrideRules`
> booking (feature 32) pushes a resource's overlap group past its capacity.
>
