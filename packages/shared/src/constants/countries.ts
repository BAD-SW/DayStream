/**
 * ISO 3166-1 country list (English names) with ISO2 code and phone dial code.
 * Shared between client (Country dropdown, phone country-code prefix) and server
 * (phone-format validation during customer creation/import) so both sides agree
 * on the same name ↔ ISO2 mapping.
 */
...
export const COUNTRIES: CountryInfo[] = [
  ...
  { name: 'Congo (Republic of the)', iso2: 'CG', dialCode: '+242' },
  { name: 'Congo (DR)', iso2: 'CD', dialCode: '+243' },
...
/** Look up a country's ISO2 code from its English name (case-insensitive). */
export function countryNameToIso2(name: string | undefined | null): string | undefined {
  if (!name) return undefined;
  return NAME_TO_ISO2[name.trim().toLowerCase()];
}

/** Look up a country's dial code (e.g. "+34") from its English name. */
export function countryNameToDialCode(name: string | undefined | null): string | undefined {
  const iso2 = countryNameToIso2(name);
  return iso2 ? COUNTRIES.find((c) => c.iso2 === iso2)?.dialCode : undefined;
}

export function iso2ToCountry(iso2: string | undefined | null): CountryInfo | undefined {
