...
describe('Membership Upgrade/Downgrade & Benefits', () => {
  beforeAll(async () => {
    ...
    const { rows: catRows } = await adminPool.query(
      `INSERT INTO svc_categories (business_id, name) VALUES ($1, 'Upgrade Cat') RETURNING id`,
      [BUSINESS_ID],
    );
    ...
    await adminPool.query(
      `INSERT INTO mem_plan_service_access (plan_id, service_id, credit_cost, access_type) VALUES ($1, $2, 2, 'included')`,
      [BASIC_PLAN_ID, SERVICE_ID],
    );

    // Add a discount benefit to premium plan
    await adminPool.query(
      `INSERT INTO mem_plan_benefits (plan_id, benefit_type, value, description) VALUES ($1, 'discount', 15, '15% off all services')`,
      [PREMIUM_PLAN_ID],
    );
  ...
  describe('PUT /memberships/:id/upgrade', () => {
    ...
    it('rejects upgrade without valid path', async () => {
      ...
      const { rows: randomPlan } = await adminPool.query(
        `INSERT INTO mem_plans (business_id, name, plan_type, billing_cycle, price) VALUES ($1, 'Random', 'unlimited', 'monthly', 20000) RETURNING id`,
        [BUSINESS_ID],
      );
  ...
  describe('Benefits evaluation', () => {
    ...
    it('detects insufficient credits', async () => {
      ...
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 1 WHERE id = $1', [MEMBERSHIP_ID]);

      const result = await benefitsService.evaluateBenefitsForBooking(CUSTOMER_ID, BUSINESS_ID, SERVICE_ID);
      expect(result.insufficient_credits).toBe(true);

      // Reset
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 10 WHERE id = $1', [MEMBERSHIP_ID]);
    });
    ...
    it('checks service exclusivity', async () => {
      ...
      const { rows: excSvc } = await adminPool.query(
        `INSERT INTO svc_services (business_id, category_id, name, slug, status, created_by)
         VALUES ($1, (SELECT id FROM svc_categories WHERE business_id = $1 LIMIT 1), 'Exclusive Service', 'exclusive-service', 'active', '00000000-0000-0000-0000-000000000010')
         ON CONFLICT (business_id, slug) DO UPDATE SET name = 'Exclusive Service' RETURNING id`,
        [BUSINESS_ID],
      );

      await adminPool.query(
        `INSERT INTO mem_plan_service_access (plan_id, service_id, credit_cost, access_type) VALUES ($1, $2, 1, 'exclusive')
    ...
    it('returns customer benefits', async () => {
      ...
      await adminPool.query('UPDATE mem_memberships SET plan_id = $2 WHERE id = $1', [MEMBERSHIP_ID, PREMIUM_PLAN_ID]);
