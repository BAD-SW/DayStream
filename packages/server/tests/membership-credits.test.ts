...
describe('Membership Credits', () => {
  ...
  describe('Rollover logic', () => {
    it('no rollover: expires all remaining credits', async () => {
      ...
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 5 WHERE id = $1', [MEMBERSHIP_ID]);
      ...
      const { rows } = await adminPool.query('SELECT credit_balance FROM mem_memberships WHERE id = $1', [MEMBERSHIP_ID]);
      expect(rows[0].credit_balance).toBe(0);
    });

    it('limited rollover: caps at max', async () => {
      // Set balance to 7 (max_rollover is 3)
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 7 WHERE id = $1', [MEMBERSHIP_ID]);
    ...
    it('unlimited rollover: keeps all', async () => {
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 8 WHERE id = $1', [MEMBERSHIP_ID]);
  ...
  describe('Credit allocation on renewal', () => {
    it('allocates new credits', async () => {
      await adminPool.query('UPDATE mem_memberships SET credit_balance = 3 WHERE id = $1', [MEMBERSHIP_ID]);
