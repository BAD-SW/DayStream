...
describe('Promotions', () => {
  beforeAll(async () => {
    ...
    await adminPool.query("DELETE FROM pricing_rules WHERE business_id = $1 AND rule_type = 'promotion'", [BUSINESS_ID]);

    // Create an active promotion
    const { rows: promo } = await adminPool.query(
      `INSERT INTO pricing_rules (business_id, name, rule_type, discount_type, discount_value, effective_from, effective_to, max_redemptions, current_redemptions)
    ...
    await adminPool.query(
      `INSERT INTO pricing_rules (business_id, name, rule_type, discount_type, discount_value, effective_from, effective_to, status)
  ...
  it('records redemption count', async () => {
    ...
    const { rows } = await adminPool.query('SELECT current_redemptions FROM pricing_rules WHERE id = $1', [PROMO_ID]);
  ...
  it('expires past-due promotions', async () => {
    ...
    const { rows } = await adminPool.query(
      "SELECT status FROM pricing_rules WHERE business_id = $1 AND name = 'Old Sale'", [BUSINESS_ID],
    );
  ...
  it('activates scheduled promotions', async () => {
    ...
    const { rows } = await adminPool.query(
      "SELECT status FROM pricing_rules WHERE business_id = $1 AND name = 'Future Sale'", [BUSINESS_ID],
    );
  ...
  it('stops applying promotion at max redemptions', async () => {
    ...
    await adminPool.query('UPDATE pricing_rules SET current_redemptions = 100 WHERE id = $1', [PROMO_ID]);
