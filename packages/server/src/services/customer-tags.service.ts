...
export async function getTags(businessId: string) {
  const { rows } = await adminPool.query(
    'SELECT * FROM cus_tags WHERE business_id = $1 ORDER BY name',
    [businessId],
  );
  return rows;
}
...
export async function createTag(businessId: string, name: string, color: string) {
  const { rows } = await adminPool.query(
    'INSERT INTO cus_tags (business_id, name, color) VALUES ($1, $2, $3) RETURNING *',
...
export async function updateTag(tagId: string, businessId: string, updates: { name?: string; color?: string }) {
  ...
  const { rows } = await adminPool.query(
    `UPDATE cus_tags SET ${fields.join(', ')} WHERE id = $${idx++} AND business_id = $${idx} RETURNING *`,
    values,
  );
  return rows[0] || null;
}
...
export async function deleteTag(tagId: string, businessId: string) {
  const { rowCount } = await adminPool.query(
    'DELETE FROM cus_tags WHERE id = $1 AND business_id = $2',
    [tagId, businessId],
  );
  return (rowCount ?? 0) > 0;
}
...
export async function assignTag(customerId: string, tagId: string, assignedBy: string) {
  await adminPool.query(
    'INSERT INTO cus_customer_tags (customer_id, tag_id, assigned_by) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING',
...
export async function removeTag(customerId: string, tagId: string) {
  await adminPool.query(
    'DELETE FROM cus_customer_tags WHERE customer_id = $1 AND tag_id = $2',
    [customerId, tagId],
  );
}
...
export async function getCustomerTags(customerId: string) {
  const { rows } = await adminPool.query(
    `SELECT t.id, t.name, t.color, ct.assigned_at
     FROM cus_customer_tags ct
     JOIN cus_tags t ON t.id = ct.tag_id
