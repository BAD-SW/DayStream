...
export async function getSchedules(resourceId: string) {
  const { rows: schedules } = await adminPool.query(
    `SELECT * FROM resource_schedules WHERE resource_id = $1 ORDER BY effective_from DESC`, [resourceId]);

  for (const s of schedules) {
    const { rows: slots } = await adminPool.query(
      `SELECT * FROM resource_schedule_slots WHERE schedule_id = $1 ORDER BY day_of_week, start_time`, [s.id]);
    s.slots = slots;
  }
  return schedules;
}
...
export async function createSchedule(resourceId: string, input: {
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO resource_schedules (resource_id, name, effective_from, effective_to)
    ...
    for (const slot of input.slots) {
      await client.query(
        `INSERT INTO resource_schedule_slots (schedule_id, day_of_week, start_time, end_time)
         VALUES ($1, $2, $3, $4)`, [schedule.id, slot.dayOfWeek, slot.startTime, slot.endTime]);
    }
    await client.query('COMMIT');

    const { rows: slots } = await adminPool.query(
      `SELECT * FROM resource_schedule_slots WHERE schedule_id = $1 ORDER BY day_of_week, start_time`, [schedule.id]);
...
export async function updateSchedule(id: string, resourceId: string, updates: {
  ...
  try {
    ...
    if (fields.length > 0) {
      ...
      await client.query(
        `UPDATE resource_schedules SET ${fields.join(', ')} WHERE id = $${idx++} AND resource_id = $${idx}`, values);
    }

    if (updates.slots !== undefined) {
      await client.query(`DELETE FROM resource_schedule_slots WHERE schedule_id = $1`, [id]);
    ...
    const { rows } = await adminPool.query(`SELECT * FROM resource_schedules WHERE id = $1`, [id]);
    if (rows.length === 0) return null;
    const { rows: slots } = await adminPool.query(
      `SELECT * FROM resource_schedule_slots WHERE schedule_id = $1 ORDER BY day_of_week, start_time`, [id]);
    rows[0].slots = slots;
    return rows[0];
  } catch (err) {
    await client.query('ROLLBACK');
...
export async function getScheduleBlocks(resourceId: string, startDate?: string, endDate?: string) {
  let query = `SELECT * FROM resource_schedule_blocks WHERE resource_id = $1`;
...
export async function addScheduleBlock(resourceId: string, input: {
  ...
  const { rows } = await adminPool.query(
    `INSERT INTO resource_schedule_blocks (resource_id, block_date, start_time, end_time, reason)
...
export async function deleteScheduleBlock(id: string, resourceId: string) {
  const { rowCount } = await adminPool.query(
    `DELETE FROM resource_schedule_blocks WHERE id = $1 AND resource_id = $2`, [id, resourceId]);
  return (rowCount ?? 0) > 0;
}
...
export async function getEffectiveSchedule(resourceId: string, date: string): Promise<Array<{ start_time: string; end_time: string }>> {
  ...
  const { rows: resRows } = await adminPool.query(`SELECT is_24_7 FROM resources WHERE id = $1`, [resourceId]);
  ...
  const { rows: slots } = await adminPool.query(
    `SELECT start_time, end_time FROM resource_schedule_slots
