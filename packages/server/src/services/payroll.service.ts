...
export async function openPayPeriod(input: PayPeriodInput) {
  ...
  const { rows: existing } = await adminPool.query(
    'SELECT id FROM pay_periods WHERE business_id = $1 AND period_start = $2 AND period_end = $3',
  ...
  const { rows } = await adminPool.query(
    `INSERT INTO pay_periods (business_id, period_start, period_end)
...
export async function getPayPeriods(businessId: string) {
  const { rows } = await adminPool.query(
    'SELECT * FROM pay_periods WHERE business_id = $1 ORDER BY period_start DESC',
    [businessId],
  );
  return rows;
}
...
export async function runPayroll(periodId: string, businessId: string, tenantId: string): Promise<{ entries: any[]; totals: any }> {
  ...
  const { rows: periodRows } = await adminPool.query(
    "SELECT * FROM pay_periods WHERE id = $1 AND business_id = $2 AND status = 'open'",
    [periodId, businessId],
  );
  ...
  // Update status
  await adminPool.query("UPDATE pay_periods SET status = 'processing' WHERE id = $1", [periodId]);
  ...
  const { rows: staffWithRules } = await adminPool.query(
    `SELECT DISTINCT cr.user_id FROM compensation_rules cr
  ...
  for (const staff of staffWithRules) {
    ...
    await adminPool.query(
      "DELETE FROM payroll_entries WHERE pay_period_id = $1 AND user_id = $2 AND status = 'draft'",
      [periodId, userId],
    );

    // Insert payroll entry
    const { rows: entryRows } = await adminPool.query(
      `INSERT INTO payroll_entries (pay_period_id, user_id, hours_worked, sessions_delivered, revenue_generated, gross_pay, total_deductions, net_pay, breakdown)
...
export async function finalizePayroll(periodId: string, businessId: string, userId: string, tenantId: string): Promise<boolean> {
  const { rows } = await adminPool.query(
    "SELECT * FROM pay_periods WHERE id = $1 AND business_id = $2 AND status = 'processing'",
    [periodId, businessId],
  );
  ...
  // Finalize entries
  await adminPool.query(
    "UPDATE payroll_entries SET status = 'finalized' WHERE pay_period_id = $1",
    [periodId],
  );

  // Finalize period
  await adminPool.query(
    "UPDATE pay_periods SET status = 'finalized', finalized_at = NOW(), finalized_by = $2 WHERE id = $1",
    [periodId, userId],
  );
...
export async function getPayrollEntries(periodId: string) {
  const { rows } = await adminPool.query(
    `SELECT pe.*, u.first_name, u.last_name FROM payroll_entries pe
