...
export async function createPolicy(input: CreatePolicyInput) {
  ...
  if (input.isDefault) {
    await adminPool.query(
      'UPDATE svc_cancellation_policies SET is_default = false WHERE business_id = $1',
      [input.businessId],
    );
  }

  const { rows } = await adminPool.query(
    `INSERT INTO svc_cancellation_policies (business_id, name, is_default, free_cancellation_hours, late_cancel_fee_type, late_cancel_fee_value, noshow_fee_type, noshow_fee_value)
...
export async function getPolicies(businessId: string) {
  const { rows } = await adminPool.query(
    'SELECT * FROM svc_cancellation_policies WHERE business_id = $1 ORDER BY is_default DESC, name',
    [businessId],
  );
  return rows;
}
...
export async function updatePolicy(id: string, businessId: string, updates: UpdatePolicyInput) {
  const { rows: existing } = await adminPool.query(
    'SELECT * FROM svc_cancellation_policies WHERE id = $1 AND business_id = $2',
    [id, businessId],
  );
  ...
  const { rows } = await adminPool.query(
    `UPDATE svc_cancellation_policies SET ${fields.join(', ')} WHERE id = $${idx++} AND business_id = $${idx}
     RETURNING *`,
    values,
  );
...
export async function deletePolicy(id: string, businessId: string): Promise<boolean> {
  ...
  await adminPool.query(
    'UPDATE svc_services SET cancellation_policy_id = NULL WHERE cancellation_policy_id = $1',
    [id],
  );

  const { rowCount } = await adminPool.query(
    'DELETE FROM svc_cancellation_policies WHERE id = $1 AND business_id = $2',
    [id, businessId],
  );
  return (rowCount ?? 0) > 0;
}
