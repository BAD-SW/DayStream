...
const UPSERT_METRIC = `
  INSERT INTO rpt_daily_metrics (tenant_id, metric_date, metric_category, metric_name, metric_value, dimensions)
  VALUES ($1, $2, $3, $4, $5, $6)
...
export async function aggregateMemberships(tenantId: string, date: string) {
  const { rows: activeRows } = await adminPool.query(
    `SELECT COUNT(*)::int AS cnt FROM mem_memberships
...
export async function aggregateCustomers(tenantId: string, date: string) {
  const { rows: totalRows } = await adminPool.query(
    `SELECT COUNT(*)::int AS cnt FROM cus_customers
