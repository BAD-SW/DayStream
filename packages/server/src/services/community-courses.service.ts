...
export async function getCourses(tenantId: string) {
  const { rows } = await adminPool.query(
    `SELECT * FROM eng_courses WHERE tenant_id = $1 AND status = 'published' ORDER BY created_at DESC`,
    [tenantId],
  );
  return rows;
}
...
export async function getCourseDetail(id: string) {
  const { rows: courses } = await adminPool.query(
    `SELECT * FROM eng_courses WHERE id = $1`,
    [id],
  );
  if (courses.length === 0) return null;

  const { rows: lessons } = await adminPool.query(
    `SELECT * FROM eng_course_lessons WHERE course_id = $1 ORDER BY display_order ASC`,
    [id],
  );
...
export async function enrollInCourse(courseId: string, customerId: string) {
  const { rows } = await adminPool.query(
    `INSERT INTO eng_course_enrollments (course_id, customer_id)
     VALUES ($1, $2)
...
export async function completeLesson(enrollmentId: string, lessonId: string) {
  ...
  await adminPool.query(
    `INSERT INTO eng_course_lesson_progress (enrollment_id, lesson_id, completed, completed_at)
     VALUES ($1, $2, true, NOW())
  ...
  const { rows: enrollments } = await adminPool.query(
    `SELECT ce.*, c.total_lessons
     FROM eng_course_enrollments ce
     JOIN eng_courses c ON c.id = ce.course_id
     WHERE ce.id = $1`,
    [enrollmentId],
  );
