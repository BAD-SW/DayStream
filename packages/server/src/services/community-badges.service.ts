...
export async function getBadges(tenantId: string) {
  const { rows } = await adminPool.query(
    `SELECT * FROM badges WHERE tenant_id = $1 ORDER BY tier, name`,
    [tenantId],
  );
  return rows;
}
...
export async function createBadge(tenantId: string, input: {
  ...
  const { rows } = await adminPool.query(
    `INSERT INTO badges (tenant_id, name, description, icon_path, tier, trigger_type, trigger_config, points_award)
