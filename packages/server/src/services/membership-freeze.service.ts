...
export async function resumeMembership(
  ...
  await adminPool.query(
    `UPDATE memberships SET status = 'active', paused_at = NULL, pause_end_date = NULL, updated_at = NOW()
     WHERE id = $1 AND business_id = $2`,
    [membershipId, businessId],
  );
...
export async function autoResumePaused(): Promise<number> {
  const { rows } = await adminPool.query(
    "SELECT id, business_id FROM memberships WHERE status = 'paused' AND pause_end_date <= CURRENT_DATE",
  );

  for (const m of rows) {
    await adminPool.query(
      "UPDATE memberships SET status = 'active', paused_at = NULL, pause_end_date = NULL, updated_at = NOW() WHERE id = $1",
      [m.id],
    );
    await adminPool.query(
