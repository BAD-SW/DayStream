...
export async function addFamilyMember(
  ...
  const { rows: countRows } = await adminPool.query(
    "SELECT COUNT(*)::int AS count FROM mem_memberships WHERE primary_membership_id = $1 AND status IN ('active', 'paused')",
    [primaryMembershipId],
  );
  ...
  const { rows: existing } = await adminPool.query(
    "SELECT id FROM mem_memberships WHERE customer_id = $1 AND plan_id = $2 AND status IN ('active', 'paused')",
  ...
  const { rows: mbrRows } = await adminPool.query(
    `INSERT INTO mem_memberships (business_id, customer_id, plan_id, status, start_date, next_billing_date, auto_renew, credit_balance, primary_membership_id, created_by)
...
export async function removeFamilyMember(
  ...
  const { rowCount } = await adminPool.query(
    "UPDATE mem_memberships SET status = 'cancelled', cancelled_at = NOW(), cancellation_reason = 'Removed from family membership'  WHERE primary_membership_id = $1 AND customer_id = $2 AND business_id = $3 AND status IN ('active', 'paused')",
...
export async function deductSharedCredit(
  ...
  const { rows } = await adminPool.query(
    'SELECT primary_membership_id FROM mem_memberships WHERE id = $1',
    [memberMembershipId],
  );
