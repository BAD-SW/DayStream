...
export async function importStatement(businessId: string, accountName: string, statementDate: string, lines: Array<{ date: string; description: string; amount: number; reference?: string }>) {
  const { rows } = await adminPool.query(
    `INSERT INTO fin_bank_statements (business_id, account_name, statement_date) VALUES ($1,$2,$3) RETURNING *`,
  ...
  for (const line of lines) {
    await adminPool.query(
      `INSERT INTO fin_bank_statement_lines (statement_id, date, description, amount, reference) VALUES ($1,$2,$3,$4,$5)`,
...
export async function getStatementLines(statementId: string) {
  const { rows } = await adminPool.query(
    `SELECT bsl.*, je.description AS matched_description
     FROM fin_bank_statement_lines bsl
     LEFT JOIN fin_journal_entries je ON je.id = bsl.matched_entry_id
...
export async function matchLine(lineId: string, journalEntryId: string): Promise<boolean> {
  const { rowCount } = await adminPool.query(
    "UPDATE fin_bank_statement_lines SET reconciliation_status = 'matched', matched_entry_id = $2, reconciled_at = NOW() WHERE id = $1",
...
export async function suggestMatches(statementId: string, businessId: string) {
  const { rows: unmatchedLines } = await adminPool.query(
    "SELECT * FROM fin_bank_statement_lines WHERE statement_id = $1 AND reconciliation_status = 'unmatched'",
    [statementId],
  );
  ...
  for (const line of unmatchedLines) {
    ...
    const { rows: candidates } = await adminPool.query(
      `SELECT je.id, je.description, je.entry_date,
              (SELECT SUM(jel.debit) FROM fin_journal_entry_lines jel WHERE jel.journal_entry_id = je.id) AS total_debit
       FROM fin_journal_entries je
       WHERE je.business_id = $1 AND je.is_void = false
         AND je.entry_date BETWEEN ($2::date - 2) AND ($2::date + 2)
         AND NOT EXISTS (SELECT 1 FROM fin_bank_statement_lines bsl WHERE bsl.matched_entry_id = je.id)
       LIMIT 5`,
...
export async function getReconciliationStatus(statementId: string) {
  const { rows } = await adminPool.query(
    `SELECT reconciliation_status, COUNT(*)::int AS count FROM fin_bank_statement_lines WHERE statement_id = $1 GROUP BY reconciliation_status`,
    [statementId],
  );
