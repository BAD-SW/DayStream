...
export async function getEntries(businessId: string, filters?: { dateFrom?: string; dateTo?: string; referenceType?: string; page?: number; limit?: number }) {
  ...
  const { rows } = await adminPool.query(
    `SELECT je.* FROM fin_journal_entries je WHERE ${where} ORDER BY je.entry_date DESC, je.created_at DESC LIMIT ${limit} OFFSET ${offset}`,
    params,
  );
  ...
  const { rows: countRows } = await adminPool.query(
    `SELECT COUNT(*)::int AS total FROM fin_journal_entries je WHERE ${where}`, params,
  );
...
export async function voidEntry(id: string, businessId: string, userId: string): Promise<{ success: boolean; reversing_entry_id?: string; error?: string }> {
  const { rows } = await adminPool.query(
    'SELECT * FROM fin_journal_entries WHERE id = $1 AND business_id = $2 AND is_void = false', [id, businessId],
  );
  ...
  const { rows: originalLines } = await adminPool.query(
    'SELECT * FROM fin_journal_entry_lines WHERE journal_entry_id = $1', [id],
  );
  ...
  await adminPool.query(
    'UPDATE fin_journal_entries SET is_void = true, void_entry_id = $2 WHERE id = $1',
